...
// ---------------------------------------------------------------------------
// SENTINEL Trust Fabric & Security APIs
// ---------------------------------------------------------------------------
...
export async function fetchSecurityStatus(): Promise<KeyStatus> {
  const res = await fetch(`${API_BASE}/security/status`, { cache: "no-store" });
  if (!res.ok) throw new Error(`GET /security/status failed: ${res.status}`);
  return res.json();
}

export async function lockEnterpriseData(actor: string = "procurement_admin"): Promise<KeyStatus> {
  const res = await fetch(`${API_BASE}/security/lock`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actor }),
  });
  if (!res.ok) throw new Error(`POST /security/lock failed: ${res.status}`);
  return res.json();
}

export async function unlockEnterpriseData(actor: string = "procurement_admin"): Promise<KeyStatus> {
  const res = await fetch(`${API_BASE}/security/unlock`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actor }),
  });
  if (!res.ok) throw new Error(`POST /security/unlock failed: ${res.status}`);
  return res.json();
}

export async function requestBreakGlass(actor: string, reason: string, duration_minutes = 15): Promise<KeyStatus> {
  const res = await fetch(`${API_BASE}/security/break-glass`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ actor, reason, duration_minutes }),
  });
  if (!res.ok) throw new Error(`POST /security/break-glass failed: ${res.status}`);
  return res.json();
}

export async function fetchSecurityAudit(): Promise<SecurityEvent[]> {
  const res = await fetch(`${API_BASE}/security/audit`, { cache: "no-store" });
  if (!res.ok) throw new Error(`GET /security/audit failed: ${res.status}`);
  return res.json();
}

export async function checkIntentFirewall(query: string): Promise<FirewallCheckResult> {
  const res = await fetch(`${API_BASE}/security/firewall/check`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ query }),
  });
  if (!res.ok) throw new Error(`POST /security/firewall/check failed: ${res.status}`);
  return res.json();
}
